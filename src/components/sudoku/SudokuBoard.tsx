...
export type SudokuBoardPropsRefs = {
	hintsRef: MutableRefObject<Mark[]>;
	marksRef: MutableRefObject<Mark[]>;
};

export const SudokuBoard = forwardRef<SudokuBoardPropsRefs, SudokuBoardProps>(
	(
		{
			className,
		...
		const _ref = ref as unknown as SudokuBoardPropsRefs;
		const marksRef = _ref?.marksRef;
		const hintsRef = _ref?.hintsRef;
		...
		const [marks, _setMarks] = useState<Mark[]>([]);
		const [hints, _setHints] = useState<Mark[]>([]);
			...
			if (shouldErasePencilMarks) {
				return setPencilMarks(
					pencilMarks.map((pencilMark) => {
						...
						return {
							...pencilMark,
							values: pencilMark.values.filter((_value) => _value !== value),
						};
					}),
				);
			}
		...
		/**
		 * Writes hints of the specified value at the selected cells
		 * @param value The value to be written
		 */
		const writeHints = (value: number) => {
			const shouldEraseHints = selectedCells.reduce((shouldErase, selectedCell) => {
				const hint = hints.find(getCellComparer(selectedCell));

				return shouldErase && hint !== undefined && hint.value === value;
			}, true);

			if (shouldEraseHints) {
				return setHints(hints.filter((hint) => !selectedCells.some(getCellComparer(hint))));
			}

			const unselectedHints: Mark[] = hints.filter(
				(hint) => !selectedCells.some(getCellComparer(hint)),
			);

			const existingHints: Mark[] = hints
				.filter((hint) => selectedCells.some(getCellComparer(hint)))
				.map((hint) => ({ ...hint, value }));

			const newHints: Mark[] = selectedCells
				.filter((selectedCell) => !hints.some(getCellComparer(selectedCell)))
				.map((cell) => ({ ...cell, value }));

			setHints([...unselectedHints, ...existingHints, ...newHints]);
		};
			...
			(e) => {
				...
				switch (mode) {
					...
					case BoardMode.Enum.EDIT:
						if (e.ctrlKey) {
							handleControlClick();
							onControlClickCell && onControlClickCell(cellContext);
						} else if (e.shiftKey) {
							handleShiftClick();
							onShiftClickCell && onShiftClickCell(cellContext);
						} else {
							handleNormalClick();
							onClickCell && onClickCell(cellContext);
						}
						break;
				}
			};
			...
			const handleNonWritableInput = () => {
				...
				switch (e.key) {
					...
					case "Control":
						...
						break;
					case "Backspace":
						if (mode === BoardMode.Enum.EDIT) {
							setHints(hints.filter((hint) => !selectedCells.some(getCellComparer(hint))));
			...
			return (
				...
				>
					{Array.from({
						...
						<span
							key={index}
							className={cn("pencil-mark-cell", "flex items-center justify-center")}
						>
	...
	props: Omit<SudokuBoardProps, "ref"> & { ref?: Partial<SudokuBoardPropsRefs> },
) => ReactNode;
